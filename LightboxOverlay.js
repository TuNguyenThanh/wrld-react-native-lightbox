...
const DRAG_DISMISS_THRESHOLD = Dimensions.get('window').height /2;
...
export default class LightboxOverlay extends Component {
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderMove: Animated.event([
          ...
          { dy: this.state.pan},
            ...
            //Set swipe down here
            if(gestureState.dy > (WINDOW_HEIGHT) /3) {
      ...
      onPanResponderRelease: (evt, gestureState) => {
        //Set swipe down gestureState.dy > 0
        if(Math.abs(gestureState.dy) > DRAG_DISMISS_THRESHOLD && gestureState.dy > 0) {
          this.setState({
            isPanning: false,
            target: {
              y: gestureState.dy,
  ...
  render() {
    ...
    const header = (<Animated.View style={[styles.header, lightboxOpacityStyle]}>{(renderHeader ?
      ...
      (
        <TouchableOpacity onPress={this.close} style={closeButtonStyle}>
